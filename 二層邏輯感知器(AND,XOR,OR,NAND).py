import numpy as np
#二層感知器
class Perceptron:
    def __init__(self, input_length, weights=None, bias=None):
        if weights is None:
            self.weights = np.ones(input_lengh) * 1
        else:
            self.weights = weights
        if bias is None:
            self.bias = -1
        else:
            self.bias = bias
    
    @staticmethod
    def activation_function(x):
        if x >0:
            return 1
        return 0
    def __call__(self, input_data):#多傳回wx+b計算結果w_sum
        w_input = self.weights * input_data
        w_sum = w_input.sum() + self.bias
        return Perceptron.activation_function(w_sum), w_sum
#建立Perceptron 物件h1, h2, o1
#------------------XOR
weights = np.array([1,1])
bias = -0.5
h1 = Perceptron(2, weights, bias)

weights = np.array([1,1])
bias = -1.5
h2 = Perceptron(2, weights, bias)

weights = np.array([1,-2])
bias = -0.5
XOR_Gate = Perceptron(2, weights, bias)

#-------------------AND
weights = np.array([1,1])
bias =-1
AND_Gate = Perceptron(2, weights, bias)

#--------------------NAND
weights = np.array([-1,-1])
bias =1.5
NAND_Gate = Perceptron(2, weights, bias)

#--------------------OR
weights = np.array([1,1])
bias =-0.5
OR_Gate = Perceptron(2, weights, bias)

#--------------------
input_data = [np.array([0,0]), np.array([0,1]),
              np.array([1,0]), np.array([1,1])]
#for 迴圈呼叫h1()和h2()函式傳回2個感知器的輸出結果和wx+b的值
print('-'*10,'XOR','-'*10)#----------------
for x in input_data:
    out1, w1 = h1(np.array(x))#輸出結果建立陣列
    out2, w2 = h2(np.array(x))#輸出結果建立陣列
    new_point = np.array([w1,w2])
    new_input = np.array([out1,out2])
    out, w = XOR_Gate(new_input)
    print(x, new_point, new_input, out)
    
print('-'*10,'AND','-'*10)#----------------
for x in input_data:
    out1, w1 = h1(np.array(x))#輸出結果建立陣列
    out2, w2 = h2(np.array(x))#輸出結果建立陣列
    new_point = np.array([w1,w2])
    new_input = np.array([out1,out2])
    out, w = AND_Gate(new_input)
    print(x, new_point, new_input, out)
    
print('-'*10,'OR','-'*10)#----------------
for x in input_data:
    out1, w1 = h1(np.array(x))#輸出結果建立陣列
    out2, w2 = h2(np.array(x))#輸出結果建立陣列
    new_point = np.array([w1,w2])
    new_input = np.array([out1,out2])
    out, w = OR_Gate(new_input)
    print(x, new_point, new_input, out)
    
print('-'*10,'NAND','-'*10)#----------------
for x in input_data:
    out1, w1 = h1(np.array(x))#輸出結果建立陣列
    out2, w2 = h2(np.array(x))#輸出結果建立陣列
    new_point = np.array([w1,w2])
    new_input = np.array([out1,out2])
    out, w = NAND_Gate(new_input)
    print(x, new_point, new_input, out)
    